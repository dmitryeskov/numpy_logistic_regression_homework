...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        scores = X.dot(self.w)
        proba_class_1 = 1.0 / (1.0 + np.exp(-scores))
        proba_class_0 = 1.0 - proba_class_1

        y_proba = np.vstack((proba_class_0, proba_class_1)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        scores_safe = np.clip(scores, -100, 100)

        sigmoid = 1.0 / (1.0 + np.exp(-scores_safe))

        epsilon = 1e-12
        log_probs = y_batch * np.log(sigmoid + epsilon) + (1 - y_batch) * np.log(1 - sigmoid + epsilon)
        loss = -np.sum(log_probs) / X_batch.shape[0]
        ...
        loss += 0.5 * reg * np.sum(self.w[:-1] * self.w[:-1])
        dw[:-1] += reg * self.w[:-1]
